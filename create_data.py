...
pedidos_semanais = pedidos_semanais[['Código','Descrição','Subcategoria']].rename(columns={'Descrição': 'DESCRICAO','Subcategoria':'SUBCATEGORIA','Código':'SKU'})
df = pd.concat([df, pedidos_semanais])
df = df.drop_duplicates(subset=['DESCRICAO'])
df = pd.merge(df, pedidos_semanais, on=['DESCRICAO'], how='left')
df = df.drop(columns=['SUBCATEGORIA_x','SKU_y']).rename(columns={'SUBCATEGORIA_y':'SUBCATEGORIA','SKU_x':'SKU'})
df = df.drop_duplicates(subset=['DESCRICAO'])
